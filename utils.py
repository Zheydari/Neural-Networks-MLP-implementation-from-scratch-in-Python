...
def load_mnist_trainval():
    ...
    if 0 in {len_data, len_label}:
        raise ValueError("Arrays must have length greater than 0")
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    #    batch size                                                             #
    #############################################################################
    #data after trail val split
    if shuffle:
        random_index = list(np.random.permutation(len(data))) ## Returns permuted range to keep data in line
        data_arr = np.array(data)
        label_arr = np.array(label)
        data = list(data_arr[random_index])
        label = list(label_arr[random_index])

    ##  Inspired from https://www.geeksforgeeks.org/break-list-chunks-size-n-python/

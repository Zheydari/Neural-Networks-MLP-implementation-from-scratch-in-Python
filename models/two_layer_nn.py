...
class TwoLayerNet(_baseNetwork):
    def __init__(self, input_size=28 * 28, num_classes=10, hidden_size=128):
        ...
        self._weight_init()
        self.cls = _baseNetwork(self.input_size, self.num_classes)
    ...
    def forward(self, X, y, mode='train'):
        ...
        h1 = self.cls.sigmoid(z1)
        z2 = np.dot(h1, self.weights['W2'])+self.weights['b2']
        #o = self.cls.sigmoid(z2)
        prob = self.cls.softmax(z2)
        loss = self.cls.cross_entropy_loss(prob, y)
        y_oh = np.eye(self.num_classes)[y]
        accuracy = self.cls.compute_accuracy(prob, y)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        #############################################################################
        #dz2 = np.multiply((y_oh - prob),self.cls.sigmoid_dev(z2))
        dz2 = (y_oh - prob)
        self.gradients['W2'] = (-1/m)*np.dot(h1.T,dz2)
        self.gradients['b2'] = (-1/m)*np.sum(dz2,axis=0,keepdims=True)
        dz1 = np.multiply((dz2@(self.weights['W2'].T)),self.cls.sigmoid_dev(z1))
        self.gradients['W1'] = (-1/m)*np.dot(X.T,dz1)
        self.gradients['b1'] = (-1/m)*np.sum(dz1,axis=0,keepdims=True)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
